...
const allUpcomingInternships: Internship[] = Array.from({ length: 30 }, (_, i) => ({
  id: i + 1,
  title: `Upcoming Internship ${i + 1}`,
  duration: `${2 + (i % 3)} Months`,
  expectedStart: `0${(i % 12) + 1} May 2026`,
  description: `Description for Upcoming Internship ${i + 1}. Learn valuable skills.`,
  image: "https://images.unsplash.com/photo-1522202176988-66273c2fd55f",
}));

export default function UpcomingInternshipsPage() {
  ...
  const filteredInternships = useMemo(() => {
    return allUpcomingInternships.filter((intern) => {
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-12 text-black">
      ...
      <div className="mb-8 text-center">
        <h1 className="text-3xl font-bold">Upcoming Internships</h1>
        <p className="mt-2 text-sm text-gray-600">All upcoming internship programs</p>
      </div>
        ...
        {paginated.map((intern) => (
          ...
          >
            ...
            <div className="p-5">
              ...
              <p className="text-sm text-gray-500">
                <strong>Expected Start:</strong> {intern.expectedStart}
              </p>
            </div>
          </div>
        ))}
        ...
        <div className="mt-8 flex justify-center gap-2">
          {Array.from({ length: totalPages }, (_, i) => (
            <button
              ...
              className={`px-3 py-1 rounded-md border font-medium transition ${currentPage === i + 1
